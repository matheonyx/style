document.addEventListener('DOMContentLoaded', function () {
  ...
  function reelsToggleMute2() {

    const vid = document.getElementById('reels-reelVideo2');

    const muteIcon = document.getElementById('reels-muteIcon2');
  ...
  function reelsToggleContent2() {

    const container = document.querySelector('.reels-clone2');

    if (!container) return;

    const content = document.getElementById('reels-content2');
    ...
    if (content.classList.contains('active')) {
      ...
      document.addEventListener('keydown', escKeyHandler2);
      ...
      document.removeEventListener('keydown', escKeyHandler2);
  ...
  function outsideClickHandler2(e) {

    const content = document.getElementById('reels-content2');

    const toggleBtn = document.querySelector('.reels-menu-toggle-btn2');
  ...
  function reelsToggleEmojiPicker2() {

    const emojiPicker = document.getElementById('reels-emojiPicker2');
  ...
  function reelsAddEmoji2(emoji) {

    const commentInput = document.getElementById('reels-commentInput2');
    ...
    reelsToggleEmojiPicker2(); // Ferme le sélecteur après ajout

  }

  // ✅ Soumet un commentaire

  function reelsSubmitComment2() {

    const commentInput = document.getElementById('reels-commentInput2');

    const commentsSection = document.getElementById('reels-comments2');

    if (!commentInput || !commentsSection || !commentInput.value.trim()) return;

    const newComment = document.createElement('div');

    newComment.className = 'reels-comment2';

    newComment.innerHTML = `<strong>utilisateur</strong> ${commentInput.value}<span class="reels-timestamp2">Maintenant</span>`;
  ...
  window.reelsToggleMute2 = reelsToggleMute2;

  window.reelsToggleContent2 = reelsToggleContent2;

  window.reelsToggleEmojiPicker2 = reelsToggleEmojiPicker2;

  window.reelsAddEmoji2 = reelsAddEmoji2;

  window.reelsSubmitComment2 = reelsSubmitComment2;
